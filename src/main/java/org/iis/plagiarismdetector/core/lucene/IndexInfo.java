...
public class IndexInfo {
    ...
    static final org.apache.log4j.Logger log = org.apache.log4j.Logger
            .getLogger(IndexInfo.class.getName());
    ...
    private TermStats[] downTermsDF = null;
    private TermStats[] downTermsTF = null;
    ...
    public Long getTotalTF_OverAllFields(BytesRef text) {
        ...
        try {
            ...
            while (fieldIterator.hasNext()) {
                ...
                /*
                 * final Terms terms = MultiFields.getTerms(ireader,field); if
                 * (terms != null) { final TermsEnum termsEnum =
                 * terms.iterator(null); if (termsEnum.seekExact(text)) { return
    ...
    /**
     *
     * <code>getDocumentLength</code> extracts the length of the given field the
     * given document
     *
     * @param docId
     * @param field field's name
    ...
    public Long getDocumentLength(int docId, String field) {
        ...
        try {
            ...
            if (terms != null && terms.size() > 0) {
                ...
                // for this field
                BytesRef t = null;
                while ((t = termsEnum.next()) != null) {// explore the terms for
                    // this field
                    DocsEnum docsEnum = termsEnum.docs(null, null); // enumerate
                    // through
                    // documents,
    ...
    public ArrayList<String> getDownTerms_DF(String field, int numTerms) {
        ArrayList<String> down_DF = new ArrayList<String>();
        if (downTermsDF == null) {
            LowFreqTerms HIT = new LowFreqTerms(ireader);
            downTermsDF = HIT.getLowDFTerms(numTerms, field);
        }
        for (TermStats ts : downTermsDF) {
            down_DF.add(ts.getTermText());
        }
        return down_DF;
    }
